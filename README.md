...
今日作战台是桌面大屏驾驶舱，优先利用侧边栏之外的完整工作区。

1. 首页默认只展示关键摘要：分数、风险、今日重点、任务进度和模块概览。
2. 核心区为三栏：今日重点、任务摘要、风险提醒。
3. 今日任务默认只展示真正需要当天执行的分类：赚钱、学习、身体、复盘。
4. 任务摘要分类卡片支持点击筛选，默认展示未完成任务 Top 5。
5. 完整任务列表可点击“展开全部任务”查看。
6. 底部概览放大为横向卡片：今日督促、运营概览、身体概览、资金概览、复盘状态。
7. 资金不再作为每日默认任务，资金状态通过资金概览和资金页查看。
8. 资金概览支持隐藏金额，方便截图展示。
9. 详情操作去对应页面处理，首页减少内部滚动，优先使用页面整体布局。
10. 桌面端优先，手机端只保证基本可访问。

## 督促页说明

V1.1 新增“督促”页，默认包含 4 项：养号、学习、复盘、睡前收尾。

1. 当前只做网页内提醒。
2. 网页关闭时不会提醒。
3. 不请求浏览器通知权限，不使用 Push API，不播放声音。
4. 强提醒建议配合手机或电脑闹钟。
5. “睡觉”已调整为“睡前收尾”，用于提醒停止刷屏、收尾、洗漱、关电脑。
6. 点击“15分钟后提醒”不会增加已提醒次数。
7. 稍后提醒必须到达 `snoozedUntil` 才会再次触发。
8. 每个提醒时间点只计入一次，刷新页面或切换页面不会重复计数。
9. 提醒时间使用 24 小时制 `HH:mm`，例如 `13:30, 16:30, 21:00`。
10. 起床不再作为网页内提醒卡片。
11. 起床只保留目标追踪：目标起床、实际起床和起床状态。
12. 起床目标默认 12:00，最终目标默认 09:30。
13. 连续稳定后，可以在督促页手动点击“目标提前 15 分钟”。
14. 提醒时间在主界面以标签显示，点击“编辑时间”后再修改。
15. 备注默认折叠，点击“备注”后再填写。
16. “提醒中 / 已关闭”用于控制该事项是否提醒；已关闭时不会触发网页内提醒。
17. 督促规则、每日督促状态、起床目标和每日起床状态都保存在 localStorage。
18. JSON 导入导出包含督促数据和起床目标数据。
19. 云同步 UI 默认隐藏。

## 身体页说明

身体页只有两种展示模式：“展示模式：完整”和“展示模式：公开”。

1. 展示模式：公开只隐藏自律状态相关 UI，不删除数据。
2. 展示模式：完整用于个人自查。
3. JSON 导出仍保留完整身体记录数据。
4. 切换展示模式会保存到 localStorage，并可随 JSON 备份恢复。
5. 自律状态包括未记录、达标、失守。
6. 系统会计算连续达标天数、距上次失守天数和本月失守次数。
7. 急救模式按当前步骤展示：延迟 15 分钟、选择替代动作、记录结果。
8. 记录结果会同步到今日自律状态和历史记录。
9. 奖励机制改为里程碑提醒，连续 3 / 7 / 14 / 30 天会出现低刺激奖励建议。
10. 展示模式：公开会隐藏自律相关内容，但不删除任何记录。
11. 历史记录只有“展开完整历史 / 收起完整历史”，不是单独展示模式。
...
4. Excel 只在本地下载，不上传。
5. 首页资金概览只做状态查看，详细资产管理、仓位判断和 Excel 导出在资金页完成。
6. 隐私模式会隐藏首页资金总额，偏高、偏低、正常数量仍可查看。
...
- 督促页：养号、学习、复盘、睡前收尾 4 项网页内提醒，以及起床目标追踪。

默认不包含登录注册、后端服务、外部 API、客户交付台、CRM 或团队协作。

## 身体页体验精修

1. 急救模式采用渐进式三步流程，默认只显示“我现在有冲动”，当前步骤会单独突出。
2. 已完成步骤会折叠成摘要，未到步骤只保留轻提示。
3. 身体备注和自律备注默认折叠。
4. 最近历史默认只显示 7 天摘要，完整历史用“展开完整历史 / 收起完整历史”控制。
5. 奖励建议只在 3 / 7 / 14 / 30 天里程碑出现，不长期平铺。
6. 展示模式：公开会隐藏全部自律相关内容，但不删除任何数据。
